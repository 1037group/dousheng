...
func (s *FavoriteServiceImpl) FavoriteAction(ctx context.Context, req *douyin_favorite.FavoriteActionRequest) (resp *douyin_favorite.FavoriteActionResponse, err error) {
	...
	klog.CtxInfof(ctx, "[FavoriteAction][res] %v", res)
	...
	if (isFavor == 1 && req.ActionType == 1) || (res == nil && req.ActionType == 2) || (isFavor == 0 && req.ActionType == 2) {
	...
	if req == nil && req.ActionType == 1 { //创建点赞
		err = db.CreateFavorite(ctx, &favorite)
		if err != nil {
			klog.CtxErrorf(ctx, err.Error())
			return nil, errno.ParamErr
		}
	}else if req != nil && req.ActionType == 1 { //某个用户已经取消了点赞，并想重新点赞
		err = db.DoFavorite(ctx, db.DB, req.UserId, req.VideoId)
		if err != nil {
			klog.CtxErrorf(ctx, err.Error())
			return nil, errno.ParamErr
		}
	} else if  else if req.ActionType == 2 { //取消点赞
