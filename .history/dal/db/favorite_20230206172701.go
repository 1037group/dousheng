...
func UpdateFavorite(ctx context.Context, tx *gorm.DB, user_id *int64, video_id *int64, actionType int32) error {
	klog.CtxInfof(ctx, "[db.UpdateFavorite] userId: %+v, toUserId: %+v, actionType: %+v", user_id, video_id, actionType)
	t := time.Now()
	return nil
}
