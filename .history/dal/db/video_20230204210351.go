...
func MGetVideosByVideoId(ctx context.Context, tx *gorm.DB, video_id []*int64) ([]*sql.Video, error) {
	klog.CtxInfof(ctx, "[MGetVideotByVideoId] video_id: %+v\n", video_id)

	var res []*sql.Video

	query := sql.SQL_VIDEO_VIDEO_ID + " = ?"
	if err := tx.WithContext(ctx).Order(sql.SQL_FAVORITE_UTIME+" desc").Find(&res, query, video_id).Error; err != nil {
