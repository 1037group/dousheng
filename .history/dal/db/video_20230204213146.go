...
func MGetVideosByVideoId(ctx context.Context, tx *gorm.DB, videoIDs []int64) ([]*sql.Video, error) {
	klog.CtxInfof(ctx, "[MGetVideotByVideoId] videoIDs: %+v\n", videoIDs)
	...
	if err := tx.WithContext(ctx).Order(sql.SQL_FAVORITE_UTIME+" desc").Where(query, videoIDs).Find(&res).Error; err != nil {
		klog.CtxInfof(ctx, "[MGetVideotByVideoId] res: %+v\n", res)
		return res, err
	}
