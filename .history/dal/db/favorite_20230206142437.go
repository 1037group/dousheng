...
func MGetVideoIdtByUserId(ctx context.Context, tx *gorm.DB, user_id *int64) ([]*sql.Video, error) {
	...
	query := sql.SQL_FAVORITE_USER_ID + " = ?"
	queryAppend := sql.SQL_FAVORITE_ISFAVORITE + " = ?"
	if err := tx.WithContext(ctx).Order(sql.SQL_FAVORITE_UTIME+" desc").Where(query, user_id).Where(queryAppend, 1).Find(&midRes).Error; err != nil {
