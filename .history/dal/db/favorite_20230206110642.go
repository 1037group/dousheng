...
func DoFavorite(ctx context.Context, tx *gorm.DB, user_id int64, video_id int64,utime time.Time) error {
	...
	query := sql.SQL_FAVORITE_USER_ID + " = ?"
	query1 := sql.SQL_FAVORITE_VIDEO_ID + " = ?"
	return tx.Model(sql.Favorite{}).Where(query, user_id).Where(query1, video_id).UpdateColumns(map[string]interface{}{sql.SQL_FAVORITE_ISFAVORITE:1,sql.SQL_FAVORITE_UTIME:}).Error
}
